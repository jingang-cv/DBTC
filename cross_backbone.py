...
class DynamicPosBias(nn.Module):
    ...
    def forward(self, biases):
        if self.residual:
            ...
            pos = self.pos3(self.pos2(self.pos_proj(biases)))
        return pos
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        if self.position_bias:
            self.pos = DynamicPosBias(self.dim // 4, self.num_heads//2, residual=False)
    ...
    def forward(self, q, k, v, H, W, mask=None):
        ...
        # print(H)
        B_, N, C = q.shape
        # print(q.shape)                                         #64,49,96
        assert H*W == N
        # qkv = self.qkv(x).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4).contiguous()
        # q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        q = q.reshape(B_, N, self.num_heads//2, C//(self.num_heads//2)).permute(0,2,1,3)
        k = k.reshape(B_, N, self.num_heads//2, C//(self.num_heads//2)).permute(0,2,1,3)
        v = v.reshape(B_, N, self.num_heads//2, C//(self.num_heads//2)).permute(0,2,1,3)
        # print(q.shape)   
        # print(k.shape)
        # print(v.shape)
        q = q * self.scale
        attn = (q @ k.transpose(-2, -1)) # (B, self.num_heads, N, N), N = H*W
        # print(attn.shape)  #64,3,49,49   
        if self.position_bias:
            # generate mother-set
            position_bias_h = torch.arange(1 - group_size[0], group_size[0], device=attn.device)
            # print(position_bias_h)
            position_bias_w = torch.arange(1 - group_size[1], group_size[1], device=attn.device)
            # print(position_bias_w)
            ...
            # print(biases.shape)
            l = biases.shape[0]
            # print(biases.shape)  #2,13,13
            if self.dis == 0:
                # a = torch.ones((l,1))
                a = torch.ones((l,1)).cuda()
                # print(a.shape)
            else:
                # a = torch.ones((l,1))*self.I
                a = torch.ones((l,1)).cuda()*self.I
            # print(self.I)
            biases = torch.cat([biases,a],1).cuda()
            # print(biases.shape)
            # biases = biases.flatten(1).transpose(0, 1).contiguous().float()
            # print(biases.shape)
            # print(biases.shape)   #169,2
            ...
            relative_position_index = relative_coords.sum(-1)  # Gh*Gw, Gh*Gw     #49,49
...
class CrossFormerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, group_size=7, interval=8, 
        ...
        self.attn0 = Attention(
            ...
            position_bias=True,dis = 0,I = interval)
        self.attn1 = Attention(
            dim//2, num_heads=num_heads,
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop,
            position_bias=True,dis = 1,I = interval)
    ...
    def forward(self, x, H, W):
        ...
        qkv = self.qkv(x).reshape(B, L, 3, C ).permute(2, 0, 1, 3).contiguous()
        # print(qkv.shape)
        q, k, v = qkv[0], qkv[1], qkv[2]  # make torchscript happy (cannot use tensor as tuple)
        # print(q.shape)
        q1 = q[:,:,:C//2]
        ...
        q1 = q1.view(B, H, W, C//2)
        k1 = k1.view(B, H, W, C//2)
        v1 = v1.view(B, H, W, C//2)
        ##### 前一半Heads算local的
        _, Hp, Wp, _ = q1.shape
        G_local = Gh_local = Gw_local = self.group_size                                 
        q1 = q1.reshape(B, Hp // G_local, G_local, Wp // G_local, G_local, C//2).permute(0, 1, 3, 2, 4, 5).contiguous()
        q1 = q1.reshape(B * Hp * Wp // G_local**2, G_local**2, C//2)
    
        v1 = v1.reshape(B, Hp // G_local, G_local, Wp // G_local, G_local, C//2).permute(0, 1, 3, 2, 4, 5).contiguous()
        v1 = v1.reshape(B * Hp * Wp // G_local**2, G_local**2, C//2)
        # print(v1.shape)
        k1 = k1.reshape(B, Hp // G_local, G_local, Wp // G_local, G_local, C//2).permute(0, 1, 3, 2, 4, 5).contiguous()
        k1 = k1.reshape(B * Hp * Wp // G_local**2, G_local**2, C//2)
        # print(q1.shape)
        # print(q1.shape)
        ##### 后一半Heads算nonlocal的
        I, Gh_nonlocal, Gw_nonlocal = self.interval, Hp // self.interval, Wp // self.interval      #8,7,7
        # print(I,Gh,Gw)
        q2 = q2.view(B, H, W, C//2)
        k2 = k2.view(B, H, W, C//2)
        v2 = v2.view(B, H, W, C//2)
        q2 = q2.reshape(B, Gh_nonlocal, I, Gw_nonlocal, I, C//2).permute(0, 2, 4, 1, 3, 5).contiguous()
        q2 = q2.reshape(B * I * I, Gh_nonlocal * Gw_nonlocal, C//2)
        k2 = k2.reshape(B, Gh_nonlocal, I, Gw_nonlocal, I, C//2).permute(0, 2, 4, 1, 3, 5).contiguous()
        k2 = k2.reshape(B * I * I, Gh_nonlocal * Gw_nonlocal, C//2)
        v2 = v2.reshape(B, Gh_nonlocal, I, Gw_nonlocal, I, C//2).permute(0, 2, 4, 1, 3, 5).contiguous()
        v2 = v2.reshape(B * I * I, Gh_nonlocal * Gw_nonlocal, C//2) 
        # print(q2.shape)
        x1 = self.attn0(q1, k1, v1, Gh_local, Gw_local)
        x2 = self.attn1(q2, k2, v2, Gh_nonlocal, Gw_nonlocal)
        x1 = x1.reshape(B, Hp // G_local, Wp // G_local, G_local, G_local, C//2).permute(0, 1, 3, 2, 4, 5).contiguous()
        x1 = x1.reshape(B, Hp, Wp, C//2)
        x1 = x1.view(B, H * W, C//2)
        
        x2 = x2.reshape(B, I, I, Gh_nonlocal, Gw_nonlocal, C//2).permute(0, 3, 1, 4, 2, 5).contiguous()
        x2 = x2.reshape(B, Hp, Wp, C//2)
        x2 = x2.view(B, H * W, C//2)
...
class PatchEmbed_cross(nn.Module):
...
class CrossFormer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=[4, 8, 16, 32], in_chans=3, num_classes=1000,
                 ...
                 group_size=[7,7,7,7], crs_interval=[8, 4, 2, 1], mlp_ratio=4., qkv_bias=True, qk_scale=None,
